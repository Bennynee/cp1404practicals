...
# use a list comprehension and the join string method to create a string (not list) of the last names for those full names longer than 11 characters
# the result should be: 'Harlem, Hendrix, Lovelace'
last_names_longer_than_11_chars = ', '.join([name.split()[1] for name in full_names if len(name) > 11])
print(last_names_longer_than_11_chars)
